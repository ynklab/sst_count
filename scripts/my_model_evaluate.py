...
class Model(object):
    ...
    :type domain: list (changed from set) 
    ...
    def __init__(self, domain, valuation):
        ...
        self.domain = sorted(list(self.domain), key = cmp_to_key(variable_compare))
    ...
    def evaluate(self, expr, g, trace=None):
        ...
        try:
            ...
            value = self.satisfy(parsed, self.domain, g, trace=trace)
            if trace:
                print()
    ...
    def satisfy(self, parsed, domain, g, trace=None):
        """
        ...
        if isinstance(parsed, ApplicationExpression):
            ...
            if isinstance(function, AbstractVariableExpression):
                ...
                funval = self.satisfy(function, domain, g)
                argvals = tuple(self.satisfy(arg, domain, g) for arg in arguments)
                return argvals in funval
            else:
                #It must be a lambda expression, so use curried form
                funval = self.satisfy(parsed.function, domain, g)
                argval = self.satisfy(parsed.argument, domain, g)
                return funval[argval]
        elif isinstance(parsed, NegatedExpression):
            return not self.satisfy(parsed.term, domain, g)
        elif isinstance(parsed, AndExpression):
            return self.satisfy(parsed.first, domain, g) and \
                   self.satisfy(parsed.second, domain, g)
        elif isinstance(parsed, OrExpression):
            return self.satisfy(parsed.first, domain, g) or \
                   self.satisfy(parsed.second, domain, g)
        elif isinstance(parsed, ImpExpression):
            return (not self.satisfy(parsed.first, domain, g)) or \
                   self.satisfy(parsed.second, domain, g)
        elif isinstance(parsed, IffExpression):
            return self.satisfy(parsed.first, domain, g) == \
                   self.satisfy(parsed.second, domain, g)
            ...
            for u in self.domain:
                ...
                if not self.satisfy(parsed.term, domain, new_g):
                    return False
            return True
        elif isinstance(parsed, ExistsExpression):
            new_g = g.copy()
            for u in domain:
                # Entity variables always begin with 'X' or 'x'
                # Event variables always begin with 'V' or 'v'
                if parsed.variable.name[0] == u.lower()[0]:
                    new_g.add(parsed.variable.name, u)
                    sub_domain = domain[:]
                    if u[0] == 'X': sub_domain.remove(u)
                    if self.satisfy(parsed.term, sub_domain, new_g):
                        return True
            ...
            for u in self.domain:
                val = self.satisfy(parsed.term, domain, g.add(var, u))
